...
ServerEvent::ServerEvent() : _events_arr(NULL), _events_size(0), _events_capacity(5) {
	init();
}
...
ServerEvent::~ServerEvent() {
	...
	if (_events_contexts.size() > 0) {
		...
		while (it != _events_contexts.end()) {
			IEventContext* event_context = it->second;
            std::map<int, IEventContext*>::iterator to_unregister = it;
			++it;
			unregister_event(to_unregister->second->get_fd()->get_fd());
